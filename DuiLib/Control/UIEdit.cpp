	...
	LRESULT CEditWnd::OnChar(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		if (m_pOwner->IsFloatPointOnly( ))
		{
			if ('.' == wParam)
			{
				...
				{
					ShowBalloonTip(L"Input error", L"小数点前不能为空，请先输入整数部分", TTI_ERROR);
					bHandled = TRUE;
				}
				else if (m_pOwner->m_sText.Find(_T('.')) != -1) // 检查是否已经输入了小数点  
				{
					ShowBalloonTip(L"不能输入字符", L"只能输入一个小数点", TTI_ERROR);
					bHandled = TRUE;
				}
				else
				{
